...
public class AuthController {
    ...
    public ResponseEntity<SimpleMessage> register(@RequestBody RegisterRequest user){
        return new ResponseEntity<>(service.register(user), HttpStatus.CREATED);
    }

    @PostMapping("/authenticate")
    public ResponseEntity<SimpleMessage> authenticate(@RequestBody AuthRequest request){
